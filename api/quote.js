export default async function handler(req, res) {
  ...
  try {
    if (symbol === "USDKRW") {
      ...
      const box =
        html.match(/<h3 class="h_lst"><span class="blind">미국 USD<\/span><\/h3>[\s\S]*?<div class="head_info[^>]*>([\s\S]*?)<\/div>/) ||
    ...
    if (isKorea) {
      ...
      const priceText =
        (html.match(/<p class="no_today">[\s\S]*?<span class="blind">([\d,]+)<\/span>/) || [])[1] ||
        (html.match(/<strong class="tah p11">([\d,]+)<\/strong>/) || [])[1];

      if (!priceText) {
        return res.status(500).json({
          error: "NAVER_PRICE_PARSE_FAIL",
          symbol,
          code
        });
      }
